...
export class QuestionPage {
  ...
  constructor(
    ...
    if (
      ...
      this.prev_first_opt =
        this.thisQuestion["options"].length == 0
          ? ""
          : this.thisQuestion["options"][0];
    } else {
      this.prev_first_opt = "";
    }
  }
  ...
  addQuestion() {
    ...
    this.thisQuestion["message"] = this.question
      ? this.question.value
      : "Missing Question";
    ...
    if (
      ...
      if (this.deleteFirstOptFlag) {
        ...
        this.thisQuestion["options"] = this.thisQuestion["options"].splice(
          0,
          1
        );
      }
    ...
    if (
      this.thisQuestion["options"] &&
      this.thisQuestion["options"].length == 1
    ) {
    ...
    if (valid) {
      ...
      if (this.navParams.get("question_data")) {
        ...
        this.navCtrl.getPrevious().data.qID =
          this.navParams.get("qID_fromEdit");
      }
  ...
  removeOption(idx) {
    if (
      idx == "firstOpt_dropdown" ||
      idx == "firstOpt_checkbox" ||
      idx == "firstOpt_multipleChoice"
    ) {
  ...
  checkAllChanges() {
    ...
    if (
      this.type == "multipleChoice" &&
      this.prev_first_opt == this.firstOpt_multipleChoice.value &&
      this.prev_anArray == JSON.stringify(this.anArray)
    ) {
      exitFlag = true;
    } else if (
      this.type == "checkbox" &&
      this.prev_first_opt == this.firstOpt_checkbox.value &&
      this.prev_anArray == JSON.stringify(this.anArray)
    ) {
      exitFlag = true;
    } else if (
      this.type == "dropdown" &&
      this.prev_first_opt == this.firstOpt_dropdown.value &&
      this.prev_anArray == JSON.stringify(this.anArray)
    ) {
      exitFlag = true;
    } else if (
      this.type == "shortAnswer" ||
      this.type == "longAnswer" ||
      this.type == "date" ||
      this.type == "time"
    ) {
      exitFlag = true;
    ...
    if (
      exitFlag &&
      this.prev_qMsg == this.question.value &&
      this.prev_isRequired == this.isRequired
    ) {
