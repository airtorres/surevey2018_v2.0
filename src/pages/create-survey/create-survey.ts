...
export class CreateSurveyPage {
    ...
    if (this.surveyTitle.value && this.surveyTitle.value != "") {
      ...
      if (isValidTitle && noDuplicate) {
        ...
        this.configService.showSimpleAlert(
          "Duplicate Title",
          "Try another title for this survey."
        );
      } else {
  ...
  checkAllChanges() {
    if (
      this.navParams.get("surveyFromTemplate") ||
      this.navParams.get("thisSurvey")
    ) {
      if (
        this.prev_survey == JSON.stringify(this.survey) &&
        this.prev_questions == JSON.stringify(this.questions) &&
        // && this.prev_description == this.surveyDescription.value && this.prev_title == this.surveyTitle.value){
        '"' + this.surveyDescription.value + '"' == this.prev_description &&
        '"' + this.surveyTitle.value + '"' == this.prev_title
      ) {
      ...
      if (
        this.prev_survey == JSON.stringify(this.survey) &&
        this.prev_questions == JSON.stringify(this.questions) &&
        this.prev_description == this.surveyDescription.value &&
        this.prev_title == this.surveyTitle.value
      ) {
        console.log("NO CHANGES MADE.");
        this.userCanLeave = true;
      } else {
