          ...
          for (const survId in res) {
            ...
            for (const responseIdx in res[survId]) {
              ...
              try {
                firebase
                  .database()
                  .ref("/responses/" + survId + "/" + newUserKey)
                  .set(res[survId][responseIdx], function (error) {
                    if (error) {
                      console.log(
                        "Not successful pushing local response to list of responses."
                      );
                    } else {
                      console.log(
                        "Local responses successfully added to responses!"
                      );
                    }
                  });
              } catch (e) {
